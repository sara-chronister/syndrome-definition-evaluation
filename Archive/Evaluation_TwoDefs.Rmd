...
join_vars <- c("Date", "EssenceID", all_of(select_fields))
...
```{r demographics_summary, echo=FALSE, message=FALSE, warning=FALSE}
pacman::p_load(gtsummary)

TEST <- bind_rows(def1, def2, .id = "id") %>%
  pivot_longer(.,
               cols = c('def1','def2'),
               names_to = 'DataSource',
               values_to = "Present"
  ...
  mutate(DataSource = case_when(
    DataSource == 'def1' ~ def1_name,
    DataSource == 'def2' ~ def2_name,
    TRUE ~ NA),
    Sex = case_when(
      Sex == "M" ~ "Male",
      Sex == "F" ~ "Female",
    ...
    Race = ifelse(c_race %in% c("Not Categorized", "Not Reported or Null", "Other Race", "Unknown"), "Unknown", c_race),
    Race = factor(Race, levels = c("American Indian or Alaska Native", 
                                   "Asian", "Black or African American", 
                                   "Multiracial", "Native Hawaiian or Other Pacific Islander", 
                                   "White", "Unknown")),
    Ethnicity = ifelse(c_ethnicity %in% c("Not Categorized", "Not Reported or Null", "Unknown"), "Unknown", c_ethnicity),
...
#   
#   list_storage[[demographics[i]]] <- TEST %>%
