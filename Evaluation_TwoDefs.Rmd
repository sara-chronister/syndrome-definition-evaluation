...
for(i in seq_along(manual_review_defs)){
  ...
  # Step 1: Formatting Data for Reviewers (IN PROGRESS)
  ...
  list[[i]] <- get(manual_review_defs[i]) %>%
    ...
    mutate(across(.cols = where(is.character), 
                  .fns = ~ str_replace_all(string=., pattern = "[^[:graph:]]", replacement = " "))) # Replace all characters that cannot be graphed with a space. 
