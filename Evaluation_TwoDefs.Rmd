...
def1 <- def1_full %>%
  select(Date, EssenceID, all_of(select_fields))  %>%
  mutate(def1 = 1) %>%
...
def2_elements_detected <- def2 %>%
  select(EssenceID, ChiefComplaintParsed, DischargeDiagnosis) %>%
  clean_ChiefComplaintParsed() %>%
  clean_DischargeDiagnosis() %>%
  mutate(CCDDclean = paste(ChiefComplaintParsed, DischargeDiagnosis)) %>%
  select(EssenceID, CCDDclean) %>%
...
output_folder <- "Output_TwoDefs\\"

# All Visits
write.csv(file = paste0(output_folder,"Two Defs All Visits.csv"),
          x = two_defs_table,
          row.names = FALSE)
...
manual_review_defs <- c("def1", "def2") # Change based on Evaluation Template #

## Establish Sampling Parameters
SampleMetric <- DefinitionInformation[["ManualReviewInformation"]] %>% pull_no_na(df=., variable = "SampleMetric")
SampleValue <- DefinitionInformation[["ManualReviewInformation"]] %>% pull_no_na(df=., variable = "SampleValue")
StratifiedSample <- DefinitionInformation[["ManualReviewInformation"]] %>% pull_no_na(df=., variable = "StratifiedSample")
StratifiedVariables <- DefinitionInformation[["ManualReviewInformation"]] %>% 
...
```{r manual review file directories, include=FALSE}
if(manual_review == TRUE){
...
for(i in seq_along(manual_review_defs)){ # for each definition specified (i) create the following filepaths
  ...
  fs::dir_create(paste0(output_folder, "Validation_Review\\",def_review_folder,"\\2_Comparison_Data"))
  fs::dir_create(paste0(output_folder, "Validation_Review\\",def_review_folder,"\\3_Consensus_Data"))
...
```{r manual reviewer files, include=FALSE}
if(manual_review == TRUE){
...
for(i in seq_along(manual_review_defs)){
  ...
  # Step 2: Create folder name for each definition undergoing manual review
  ...
  # Problem: When folks are running SyS Eval_ template, they have not yet started manual review --> so not yet time for Comparison/Consensus process --> need an additional script/button they can run when ready (Consensus_Review.Rmd).
  ...
  save(filepath, n_reviewers, select_fields,
       ReviewScaleLow, ReviewScaleHigh, jurisdiction,
       DefinitionInformation,
       file = paste0(filepath,"\\Resources\\Consensus_Review_Parameters.RData"))
  
  # # Step 4c: Copy Consensus_Review.Rmd code template to each definition validation review folder.
  file.copy(from = "SupportCode\\Consensus_Review\\Consensus_Review.Rmd",
            to = paste0(filepath))
