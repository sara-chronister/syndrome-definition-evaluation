...
demo_table <- demo_table %>%
  ...
  select(Demographic_Category, Demographic, everything(), `P-Value`, -`p-value`)
...
unigrams <- bind_rows(unigram_list, .id = "def") %>%
  mutate(def = factor(def, levels = params$queries_abbrev),
         unit = "Unigram") %>% # To preserve ordering of defs (avoid alphabetic ordering)
  group_by(def, Field) %>%
...
bigrams <- bind_rows(bigram_list, .id = "def") %>%
  mutate(def = factor(def, levels = params$queries_abbrev),
         unit = "Bigram") %>% # To preserve ordering of defs (avoid alphabetic ordering)
  group_by(def, Field) %>%
  mutate(order = row_number(),
         order = fct_rev(as.factor(order))) %>%
  select(def, field = Field, order, everything())
...
n_grams <- bind_rows(unigrams, bigrams) %>% 
  ...
  arrange(def, unit, field, n) %>%
  select(Syndrome = def, Unit = unit, Field = field, `Free-Text` = n_gram, `Number of Occurrences` = n)
