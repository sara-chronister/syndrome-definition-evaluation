...
if(params$n_queries_eval >= 2){
  ...
  all_def_summary <- syndrome_eval_list[[overlap_name]] %>%
...
if(params$n_queries_eval >= 2){
 plot(eulerr::venn(syndrome_eval_list[[overlap_name]] %>% select(all_of(params$queries_abbrev))), 
...
if(params$n_queries_eval >= 2){
 plot(eulerr::euler(syndrome_eval_list[[overlap_name]] %>% select(all_of(params$queries_abbrev))), 
      labels = TRUE, legend = list(side = "right"), quantities = TRUE)
}
```
