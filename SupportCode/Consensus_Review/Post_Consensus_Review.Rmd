...
When you are ready to discuss validation review findings and come to a consensus decision between the `r n_reviewers` participating reviewers (particularly for records where there was disagreements), access the following file:  **3_Consensus_Data/Consensus_Data.xlsx**. 
...
Examining the performance metrics by each of the `r n_reviewers` participating reviewers (before Consensus Review) can provide **initial insight** into the performance of the definition as well as how consistent review categories and ratings were across the reviewers. 
...
list_consensus[["Consensus_Data"]] <- readxl::read_excel(path = paste0("3_Consensus_Data\\Consensus_Data.xlsx")) 
...
# Write Excel File
writexl::write_xlsx(list_consensus[["Consensus_Data_Clean"]], path = "3_Consensus_Data\\Consensus_Data_Clean.xlsx")

```

```{r Write Consensus RData File}

save(list_consensus, file = "3_Consensus_Data\\Consensus.RData")
